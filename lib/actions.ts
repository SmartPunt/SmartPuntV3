...
export async function addUserBetAction(
  ...
  try {
    ...
    const raceId = Number(formData.get("race_id") || 0) || null;
    const raceRunnerId = Number(formData.get("race_runner_id") || 0) || null;
    const horseId = Number(formData.get("horse_id") || 0) || null;
    ...
    if (!source) {
      return { success: false, error: "Bet source is required." };
    }

    if (!horse || !race) {
      return { success: false, error: "Horse and race are required." };
    }

    if (!oddsTaken || Number.isNaN(oddsTaken) || oddsTaken <= 1) {
      return { success: false, error: "Valid odds are required." };
    }

    const { error } = await supabase.from("user_bets").insert({
      user_id: profile.id,
      source,
      suggested_tip_id: suggestedTipIdRaw ? Number(suggestedTipIdRaw) : null,
      calculator_tip_id: calculatorTipIdRaw ? Number(calculatorTipIdRaw) : null,
      race_id: raceId,
      race_runner_id: raceRunnerId,
      horse_id: horseId,
      horse,
