...
function parseRaceImportText(raw: string): ImportedRunner[] {
  ...
  while (i < lines.length) {
    ...
    if (!looksLikeHorseName(line, lines.slice(i + 1, i + 16))) {
      i += 1;
      continue;
    }
...
setRacePlaceTerms("top_3");
  ...
  function deletePreviewRunner(index: number) {
    ...
    setEditingPreviewRunnerIndexes((current) =>
      current
        .filter((runnerIndex) => runnerIndex !== index)
        .map((runnerIndex) => (runnerIndex > index ? runnerIndex - 1 : runnerIndex)),
    );
  }

  function togglePreviewRunnerEdit(index: number) {
    setEditingPreviewRunnerIndexes((current) =>
      current.includes(index)
        ? current.filter((runnerIndex) => runnerIndex !== index)
        : [...current, index],
    );
  }
...
<div className="mt-4 space-y-3">
  {parsedImportRunners.map((runner, index) => {
    const isEditingPreviewRunner =
      editingPreviewRunnerIndexes.includes(index);
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              type="button"
              onClick={() => togglePreviewRunnerEdit(index)}
              disabled={isPending || importingRunners}
              className="rounded-xl border border-zinc-300 bg-white px-3 py-2 text-xs font-black uppercase tracking-[0.16em] text-zinc-700 transition hover:bg-zinc-100 disabled:opacity-60"
            >
              {isEditingPreviewRunner ? "Close Edit" : "Edit"}
            </button>
