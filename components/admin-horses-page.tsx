...
export default function AdminHorsesPage({
  ...
  const horseRows = useMemo(() => {
    const rows = initialHorses.map((horse) => {
    ...
    if (sortMode === "most_used") {
      return filtered.sort(
        (a, b) => b.appearances - a.appearances || a.horse_name.localeCompare(b.horse_name),
      );
    }
    ...
    return filtered.sort((a, b) => a.horse_name.localeCompare(b.horse_name));
...
export default function AdminHorsesPage({
  ...
  const horseRows = useMemo(() => {
    ...
    if (sortMode === "most_used") {
      return filtered.sort((a, b) => b.appearances - a.appearances || a.horse_name.localeCompare(b.horse_name));
    }
          ...
          <div className="p-6 text-zinc-950">
            <div className="flex items-center justify-between gap-4">
              <div>
                <h2 className="text-xl font-semibold">Horse library</h2>
                <p className="text-sm text-zinc-500">
                  Search what’s already saved and see where each horse has been used recently.
                </p>
              </div>
              <Badge tone="amber">{horseRows.length} shown</Badge>
            </div>
