        ...
        <aside className="border-r border-slate-200 bg-white p-5">
          ...
          <Panel className="mt-6">
            <div className="space-y-3 p-4 text-sm text-slate-600">
              ...
              <div className="flex items-center justify-between">
                <span>Suggested tips</span>
                <Badge tone="green">{suggestedTips.length}</Badge>
              </div>
              <div className="flex items-center justify-between">
                <span>Watch items</span>
                <Badge tone="amber">{watchlistItems.length}</Badge>
              </div>
              <div className="flex items-center justify-between">
            ...
            <Panel>
              <form action={upsertSuggestedTip} className="space-y-4 p-5">
                ...
                <textarea
                  ...
                  className="min-h-[120px] w-full rounded-2xl border border-slate-200 px-3 py-2"
                />
                <button
                  type="submit"
                    ...
                    <div key={tip.id} className="rounded-2xl bg-slate-50 p-4">
                      ...
                      <div className="mt-3 flex gap-2">
                        <button
                          ...
                          onClick={() => setTipId(String(tip.id))}
                        >
                          Edit next save
                        </button>
                        <form action={deleteSuggestedTipAction}>
                          <input type="hidden" name="id" value={tip.id} />
                          <button className="rounded-2xl bg-red-600 px-4 py-2 text-sm text-white">
                            Delete
                          </button>
                        </form>
                      </div>
                    ...
                    <div key={item.id} className="rounded-2xl bg-slate-50 p-4">
                      ...
                      <div className="mt-3 flex gap-2">
                        <button
                          type="button"
                          className="rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm"
                          onClick={() => setWatchId(String(item.id))}
                        >
                          Edit next save
                        </button>
                    ...
                    <div key={item.id} className="rounded-2xl bg-slate-50 p-4">
                      ...
                      <div className="mt-3 flex gap-2">
                        <button
                          type="button"
                          className="rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm"
                          onClick={() => setLongId(String(item.id))}
                        >
                          Edit next save
                        </button>
