...
export default function AdminCalculatorTest({
  ...
  const [trackName, setTrackName] = useState("Randwick");
  ...
  const [formLast6, setFormLast6] = useState("1-2-3-4-5-6");
  ...
  const selectedHorse = useMemo(() => {
    ...
    return horses.find((horse) => horse.horse_name.toLowerCase() === key) || null;
  }, [horseName, horses]);

  const syntheticMeeting: Meeting = {
    id: -900001,
    meeting_name: trackName || "Test Track",
  ...
  const syntheticHorse: Horse = selectedHorse || {
    id: -900001,
    horse_name: horseName || "Test Horse",
    normalised_name: (horseName || "test horse").toLowerCase(),
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  };
  ...
  const syntheticRunner: Runner = {
    ...
    apprentice_claim_kg: apprenticeClaim ? Number(apprenticeClaim) : null,
  ...
  const scoredRunner = useMemo(() => {
    const scored = calculateRaceScores({
      ...
      horses: selectedHorse ? horses : [...horses, syntheticHorse],
          ...
          <div className="relative z-10 flex min-h-[220px] flex-col justify-between p-4 lg:min-h-[280px] lg:p-8">
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="ml-auto flex flex-wrap items-center gap-2">
                <Link href="/admin/calculator" className="rounded-2xl border border-white/15 bg-black/45 px-4 py-2 text-sm font-semibold text-white">
                  Calculator Lab
                </Link>
                <Link href="/" className="rounded-2xl border border-white/15 bg-black/45 px-4 py-2 text-sm font-semibold text-white">
                  Back to Admin
                </Link>
            ...
            <div className="mt-auto rounded-2xl bg-black/20 px-4 py-4">
              <h1 className="text-2xl font-bold sm:text-3xl lg:text-4xl">
                SmartPunt calculator testing module
              </h1>
              <p className="mt-2 text-sm text-zinc-200">
                Build a synthetic runner and inspect how each scoring factor reacts.
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="space-y-4 p-6 text-zinc-950">
              ...
              <div>
                ...
                <datalist id="horse-options">
                  {horses.slice(0, 9000).map((horse) => (
                    <option key={horse.id} value={horse.horse_name} />
                  ))}
                </datalist>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Start typing jockey..."
                  className="mt-2 w-full rounded-2xl border border-amber-200/30 px-4 py-3"
                />
                <datalist id="jockey-options">
                  {jockeyProfiles.map((jockey) => (
                    <option key={jockey.id} value={jockey.jockey_name} />
                  ))}
                </datalist>
              </div>

              <div className="grid gap-4 md:grid-cols-2">
                <input value={trainerName} onChange={(e) => setTrainerName(e.target.value)} placeholder="Trainer" className="rounded-2xl border border-amber-200/30 px-4 py-3" />
                <input value={trackName} onChange={(e) => setTrackName(e.target.value)} placeholder="Track" className="rounded-2xl border border-amber-200/30 px-4 py-3" />
              </div>

              <div className="grid gap-4 md:grid-cols-3">
                <select value={condition} onChange={(e) => setCondition(e.target.value)} className="rounded-2xl border border-amber-200/30 px-4 py-3">
                ...
                <input value={distance} onChange={(e) => setDistance(e.target.value)} placeholder="Distance" className="rounded-2xl border border-amber-200/30 px-4 py-3" />
                <input value={barrier} onChange={(e) => setBarrier(e.target.value)} placeholder="Barrier" className="rounded-2xl border border-amber-200/30 px-4 py-3" />
              </div>

              <div className="grid gap-4 md:grid-cols-2">
                <input value={weight} onChange={(e) => setWeight(e.target.value)} placeholder="Weight kg" className="rounded-2xl border border-amber-200/30 px-4 py-3" />
                <input value={apprenticeClaim} onChange={(e) => setApprenticeClaim(e.target.value)} placeholder="Apprentice claim kg" className="rounded-2xl border border-amber-200/30 px-4 py-3" />
              </div>

              <div className="grid gap-4 md:grid-cols-3">
                <input value={formLast6} onChange={(e) => setFormLast6(e.target.value)} placeholder="Last 6 e.g. 1-2-3" className="rounded-2xl border border-amber-200/30 px-4 py-3" />
                <input value={trackForm} onChange={(e) => setTrackForm(e.target.value)} placeholder="Track e.g. 4:1,1,0" className="rounded-2xl border border-amber-200/30 px-4 py-3" />
                <input value={distanceForm} onChange={(e) => setDistanceForm(e.target.value)} placeholder="Distance e.g. 6:2,1,1" className="rounded-2xl border border-amber-200/30 px-4 py-3" />
              </div>
            </div>
          </Panel>
          ...
          <Panel className="bg-white/95">
            <div className="space-y-5 p-6 text-zinc-950">
              ...
              {scoredRunner ? (
                <>
                  <div className="rounded-[24px] border border-amber-200/30 bg-amber-50 p-5">
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      <Badge tone="green">Win {scoredRunner.winPercent}%</Badge>
                      <Badge tone="blue">Place {scoredRunner.placePercent}%</Badge>
                      <Badge tone="amber">{scoredRunner.verdict}</Badge>
                    </div>
                  </div>
                ...
                <p className="text-sm text-zinc-500">Enter test inputs to calculate a score.</p>
              )}
            </div>
          </Panel>
        </div>
