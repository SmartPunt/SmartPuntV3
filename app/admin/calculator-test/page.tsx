...
import AdminCalculatorTest from "@/components/admin-calculator-test";
...
export default async function Page() {
  ...
  const [races, runners, horses, meetings, jockeyProfiles] = await Promise.all([
    fetchAllRows({
      getPage: async (from, to) => {
        const result = await supabase.from("races").select("*").range(from, to);
    ...
    fetchAllRows({
      getPage: async (from, to) => {
        const result = await supabase.from("race_runners").select("*").range(from, to);
        return { data: result.data ?? [], error: result.error };
      },
    }),
    ...
    fetchAllRows({
      getPage: async (from, to) => {
        const result = await supabase.from("meetings").select("*").range(from, to);
        return { data: result.data ?? [], error: result.error };
      },
    }),
