import Link from "next/link";import { redirect } from "next/navigation";import { getCurrentProfile } from "@/lib/auth";import { Badge, Panel } from "@/components/ui";

type SearchValue = string | string[] | undefined;type CalculatorReportSearchParams = Record<string, SearchValue>;

type Prediction = {id: number;race_id: number;runner_id: number;horse_id: number;scoring_version: string;score: number | string;rank: number;win_percent: number;place_percent: number;recent_form_score: number | string;distance_score: number | string;track_score: number | string;condition_score: number | string;barrier_score: number | string;weight_score: number | string;jockey_score: number | string;trainer_score: number | string;predicted_at: string;finishing_position: number | null;won: boolean | null;placed: boolean | null;settled_at: string | null;race?: RaceWithMeeting | null;horse?: { horse_name: string } | null;};

type RaceRow = {id: number;race_number: number;race_name: string;distance_m: number | null;meeting_id: number;status: string;};

type MeetingRow = {id: number;meeting_name: string;meeting_date: string;track_condition: string | null;};

type RaceWithMeeting = RaceRow & {meeting?: MeetingRow | null;};

type HorseRow = {id: number;horse_name: string;};

function headers() {const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;const serviceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY;

if (!supabaseUrl || !serviceRoleKey) {throw new Error("Missing Supabase service role configuration in environment variables.");}

return {supabaseUrl,headers: {apikey: serviceRoleKey,Authorization: Bearer ${serviceRoleKey},"Content-Type": "application/json",Accept: "application/json",},};}

async function serviceSelect(path: string): Promise<T[]> {const { supabaseUrl, headers: h } = headers();

const response = await fetch(${supabaseUrl}/rest/v1/${path}, {method: "GET",headers: h,cache: "no-store",});

if (!response.ok) {const text = await response.text();throw new Error(text || Service role request failed for ${path});}

return response.json();}

function first(value: SearchValue) {return Array.isArray(value) ? value[0] || "" : value || "";}

function toNumber(value: number | string | null | undefined) {const next = Number(value ?? 0);return Number.isNaN(next) ? 0 : next;}

function percent(part: number, total: number) {return total ? Math.round((part / total) * 100) : 0;}

function isoDate(value?: string | null) {if (!value) return "";if (/^\d{4}-\d{2}-\d{2}$/.test(value)) return value;const date = new Date(value);if (Number.isNaN(date.getTime())) return "";return date.toISOString().slice(0, 10);}

function formatDate(value?: string | null) {if (!value) return "Unknown date";const date = new Date(value);if (Number.isNaN(date.getTime())) return value;return date.toLocaleDateString("en-AU", {weekday: "short",day: "numeric",month: "short",year: "numeric",});}

function formatDateTime(value?: string | null) {if (!value) return "—";const date = new Date(value);if (Number.isNaN(date.getTime())) return value;return date.toLocaleString("en-AU", {day: "numeric",month: "short",year: "numeric",hour: "2-digit",minute: "2-digit",});}

function todayIso() {return new Date().toISOString().slice(0, 10);}

function pastIso(days: number) {const date = new Date();date.setDate(date.getDate() - days);return date.toISOString().slice(0, 10);}

function buildQuery(params: Record<string, string>) {const query = new URLSearchParams();Object.entries(params).forEach(([key, value]) => {if (value) query.set(key, value);});const value = query.toString();return value ? ?${value} : "";}

function raceLabel(row: Prediction) {const meeting = row.race?.meeting?.meeting_name || "Meeting";const raceNumber = row.race?.race_number ? R${row.race.race_number} : "Race";return ${meeting} ${raceNumber} ${row.race?.race_name || ""}.trim();}

function groupByRace(rows: Prediction[]) {const map = new Map<number, Prediction[]>();

rows.forEach((row) => {const existing = map.get(row.race_id) || [];existing.push(row);map.set(row.race_id, existing);});

return Array.from(map.entries()).map(([raceId, raceRows]) => ({raceId,rows: raceRows.sort((a, b) => a.rank - b.rank),meetingDate: raceRows[0]?.race?.meeting?.meeting_date || "",label: raceLabel(raceRows[0]),})).sort((a, b) => {const aTime = a.meetingDate ? new Date(a.meetingDate).getTime() : 0;const bTime = b.meetingDate ? new Date(b.meetingDate).getTime() : 0;if (bTime !== aTime) return bTime - aTime;return b.raceId - a.raceId;});}

function winner(rows: Prediction[]) {return rows.find((row) => row.finishing_position === 1) || null;}

function filterByDate(rows: Prediction[], from: string, to: string) {return rows.filter((row) => {const meetingDate = isoDate(row.race?.meeting?.meeting_date);if (!meetingDate) return true;if (from && meetingDate < from) return false;if (to && meetingDate > to) return false;return true;});}

async function fetchPredictions() {const predictions = await serviceSelect("calculator_predictions?select=*&settled_at=not.is.null&finishing_position=not.is.null&order=settled_at.desc",);

const raceIds = Array.from(new Set(predictions.map((row) => row.race_id).filter(Boolean)));const horseIds = Array.from(new Set(predictions.map((row) => row.horse_id).filter(Boolean)));

const races = raceIds.length? await serviceSelect(races?select=id,race_number,race_name,distance_m,meeting_id,status&id=in.(${raceIds.join(",")}),): [];

const meetingIds = Array.from(new Set(races.map((row) => row.meeting_id).filter(Boolean)));

const meetings = meetingIds.length? await serviceSelect(meetings?select=id,meeting_name,meeting_date,track_condition&id=in.(${meetingIds.join(",")}),): [];

const horses = horseIds.length? await serviceSelect(horses?select=id,horse_name&id=in.(${horseIds.join(",")})): [];

const raceMap = new Map(races.map((row) => [Number(row.id), row]));const meetingMap = new Map(meetings.map((row) => [Number(row.id), row]));const horseMap = new Map(horses.map((row) => [Number(row.id), row]));

return predictions.map((prediction) => {const race = raceMap.get(Number(prediction.race_id)) || null;const meeting = race ? meetingMap.get(Number(race.meeting_id)) || null : null;const horse = horseMap.get(Number(prediction.horse_id)) || null;
...
function StatCard({label,value,hint,tone = "amber",}: {label: string;value: string | number;hint: string;tone?: "amber" | "green" | "blue" | "rose" | "slate";}) {return ({label}{value}{label}{hint});}

export default async function CalculatorReportPage({searchParams,}: {searchParams?: Promise;}) {const profile = await getCurrentProfile();

if (!profile) redirect("/login");if (!["admin", "staff_admin"].includes(profile.role)) redirect("/");

const resolvedSearchParams: CalculatorReportSearchParams = searchParams? await searchParams: {};const dateFrom = first(resolvedSearchParams.from);const dateTo = first(resolvedSearchParams.to);

let predictions: Prediction[] = [];let errorMessage = "";

try {predictions = filterByDate(await fetchPredictions(), dateFrom, dateTo);} catch (error) {errorMessage =error instanceof Error ? error.message : "Unknown error loading calculator report.";}

const exportHref = /admin/calculator-report/export${buildQuery({ from: dateFrom, to: dateTo })};

if (errorMessage) {return (Calculator Report{errorMessage});}

const raceGroups = groupByRace(predictions);const totalRaces = raceGroups.length;const totalRunners = predictions.length;const topRatedRows = raceGroups.map((race) => race.rows.find((row) => row.rank === 1) || race.rows[0] || null).filter((row): row is Prediction => Boolean(row));

const topRatedWins = topRatedRows.filter((row) => row.finishing_position === 1).length;const topRatedPlaces = topRatedRows.filter((row) => row.finishing_position !== null && row.finishing_position <= 3,).length;const topThreeHitRaces = raceGroups.filter((race) =>race.rows.some((row) => row.rank <= 3 && row.finishing_position === 1),).length;const winners = raceGroups.map((race) => winner(race.rows)).filter((row): row is Prediction => Boolean(row));const avgWinnerRank = winners.length? (winners.reduce((sum, row) => sum + Number(row.rank || 0), 0) / winners.length).toFixed(1): "—";const avgWinnerScore = winners.length? Math.round(winners.reduce((sum, row) => sum + toNumber(row.score), 0) / winners.length): "—";const strongestWinner =[...winners].sort((a, b) => toNumber(b.score) - toNumber(a.score))[0] || null;const biggestMiss =[...topRatedRows].filter((row) => row.finishing_position !== null && row.finishing_position > 3).sort((a, b) => toNumber(b.score) - toNumber(a.score))[0] || null;
