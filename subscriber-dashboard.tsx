  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <div className="mx-auto max-w-7xl px-4 py-6 lg:px-8 lg:py-8">
        <div className="overflow-hidden rounded-[32px] border border-slate-200 bg-gradient-to-r from-slate-950 via-slate-900 to-slate-800 p-6 text-white shadow-xl lg:p-8">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p className="text-sm text-slate-300">Subscriber access</p>
              <h1 className="mt-2 text-4xl font-semibold tracking-tight">Suggested Tips</h1>
            ...
            <div className="p-6">
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-slate-500">Win, place, and all up plays from the head tipper.</p>
                </div>
              ...
              <div className="mt-5 grid gap-4 lg:grid-cols-2">
                {suggestedTips.map((tip: any) => (
                  <div
                    key={tip.id}
                    className="rounded-[24px] border border-slate-200 bg-slate-50/80 p-5 transition hover:-translate-y-0.5 hover:shadow-sm"
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <p className="text-sm text-slate-500">{tip.race}</p>
                        <h3 className="mt-1 text-2xl font-semibold text-slate-900">{tip.horse}</h3>
                      </div>
                    ...
                    <p className="mt-4 text-sm leading-6 text-slate-600">{tip.commentary || ""}</p>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="p-6">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-sm text-slate-500">Watchlist notes and market awareness.</p>
                  </div>
                    ...
                    <div key={item.id} className="rounded-[24px] border border-slate-200 bg-slate-50/80 p-5">
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          ...
                          <h4 className="mt-1 text-xl font-semibold">{item.horse || "Race note"}</h4>
                        </div>
                        <TipPill type={item.label} />
                      </div>

                      <p className="mt-4 text-sm leading-6 text-slate-600">{item.commentary || ""}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="p-6">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-sm text-slate-500">Future and longer-range angles worth tracking.</p>
                  </div>
                    ...
                    <div key={item.id} className="rounded-[24px] border border-slate-200 bg-slate-50/80 p-5">
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          ...
                          <h4 className="mt-1 text-xl font-semibold">{item.horse}</h4>
                        </div>
